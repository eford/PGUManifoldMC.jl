...
function make_param_true(P_true::Vector, K_true::Vector, e_true::Vector, w_true::Vector, M0_true::Vector, C_true::Vector)
  ...
  for i in 1:num_pl
      set_period(param,P_true[i],plid=i) # param[1+pl_offset(i)] = log(1+P_true[i]/RvModelKeplerian.P0)
      set_amplitude(param,K_true[i],plid=i) # param[2+pl_offset(i)] = log(1+K_true[i]/RvModelKeplerian.K0)
      set_ew(param,e_true[i],w_true[i],plid=i) # param[3+pl_offset(i)] = e_true[i]*cos(w_true[i])
      #param[4+pl_offset(i)] = e_true[i]*sin(w_true[i])
      set_mean_anomaly_at_t0(param,M0_true[i],plid=i) # param[5+pl_offset(i)] = M0_true[i]
  end
  for i in 1:length(C_true)
      set_rvoffset(param,C_true[i],obsid=i) # param[rv_offset(param,i)] = C_true[i]
  end
  return param
end
...
function make_param_true_ex1()
  ...
  param_true = [ log(1+P_true/RvModelKeplerian.P0), log(1+K_true/RvModelKeplerian.K0), e_true*cos(w_true), e_true*sin(w_true), M0_true, C_true ]
end
